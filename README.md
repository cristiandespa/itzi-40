...
Totul se modifică în `src/config.js`: `name`, `age`, `photo`, `photoPosition`, `audio`, `texts`, `questions` și `sequence`. Textele acceptă `{name}` și `{age}`. Dacă schimbi vârsta, adaptează și subtitlul „Patru decenii”. Numele inițial este „Itzi”.
...
- **Vercel:** importă repository-ul GitHub; configurația inclusă folosește `npm run build` și `dist`.
